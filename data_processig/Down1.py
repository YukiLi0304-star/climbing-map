...
class IrishClimbingRobust:
    ...
    def _convert_gridref(self, grid_ref, county_name=""):
        ...
        try:
            ...
            if grid_ref.isdigit() and len(grid_ref) == 6:
                ...
                for county, prefix in grid_prefixes.items():
                    if county.lower() in county_name.lower():
                        grid_ref = prefix + grid_ref
                        print(f"      根据郡名推断前缀: {grid_ref}")
                        break
            # 先尝试在线API转换
    ...
    def _convert_gridref_locally(self, grid_ref):
        ...
        try:
            ...
            # 爱尔兰网格基准坐标（100km方格）
            # 参考：https://en.wikipedia.org/wiki/Irish_grid_reference_system
            grid_bases = {
                'A': (0, 0), 'B': (100000, 0), 'C': (200000, 0),
                'D': (0, 100000), 'E': (100000, 100000), 'F': (200000, 100000),
                'G': (0, 200000), 'H': (100000, 200000), 'J': (200000, 200000),
                'K': (0, 300000), 'L': (100000, 300000), 'M': (200000, 300000),
                'N': (0, 400000), 'O': (100000, 400000), 'P': (200000, 400000),
                'Q': (0, 500000), 'R': (100000, 500000), 'S': (200000, 500000),
                'T': (0, 600000), 'U': (100000, 600000), 'V': (200000, 600000),
                'W': (0, 700000), 'X': (100000, 700000), 'Y': (200000, 700000),
            }
            ...
            easting = base_easting + int(easting_str) * 100  # 转换为米
            northing = base_northing + int(northing_str) * 100
            
            # 爱尔兰网格到经纬度的转换（近似）
            # 爱尔兰本岛范围：纬度 51.4°N 到 55.4°N，经度 -10.5°W 到 -5.5°W
            
            # 近似转换公式
            lat = 53.5 + (northing - 250000) / 111000  # 1度约111公里
            lon = -8.0 + (easting - 200000) / 70000   # 经度转换
            
            # 限制在合理范围内
            lat = max(51.0, min(55.5, lat))
            lon = max(-11.0, min(-5.0, lon))
    ...
    def extract_grid_ref_coords(self, wikitext):
        ...
        # 2. 转换为坐标
        coords = self._convert_gridref(grid_ref, county_name="Down")
        if coords:
